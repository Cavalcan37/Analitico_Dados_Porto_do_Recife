...
# 13.3.CALCULAR A PROB(CG > 38000) NO MÊS 1 (SÓ PARA O MÊS 1)
# Configurar a janela gráfica para exibir apenas um gráfico
par(mfrow = c(1,1))

# Selecionar os dados referentes ao mês 1
dados_mes1 <- scenarios[, 1]

# Calcular a probabilidade de demanda inferior a 36400
limite <- 36400
...
text(x = limite + 0.25 * diff(range(dados_mes6)), # Posição o texto da probabilidade
     y = max(dens$y) * 0.7, 
     labels = paste("P(X > 38000) =", round(probabilidade, 4)), 
     col = "red", font = 2)

# Adicionar o valor da média no gráfico
text(x = media - 0.20 * diff(range(dados_mes6)), # Posição do texto da média
     y = max(dens$y) * 0.9, 
     labels = paste("Média:", round(media, 2)), 
     col = "blue", font = 2)
