...
# 13.1. HISTOGRAMA DE PREVISÃO PARA O MÊS i
# Simular 5000 erros para cada um dos 3 períodos de previsão
set.seed(123)
num_simulations <- 5000
errors_simulated <- matrix(nrow = num_simulations, ncol = 12)
for (i in 1:1) {
  errors_simulated[, i] <- simulate_errors(num_simulations, 0, 33000)
}
...
par(mfrow = c(1,1)) # Configurar a janela gráfica para 3 colunas
for (i in 1:1) {
  hist(scenarios[, i], main = paste("Mês", i),
       xlab = "Movimentação GS (t)", col = "blue", border = "black", breaks = 20,
...
text(x = limite + 0.17 * diff(range(dados_mes1)), # Posição o texto da probabilidade
     y = max(dens$y) * 0.7, 
     labels = paste("Prob:", round(probabilidade, 4)), 
     col = "red", font = 1)

# Adicionar o valor da média no gráfico
text(x = media - 0.15 * diff(range(dados_mes6)), # Posição do texto da média
     y = max(dens$y) * 0.9, 
     labels = paste("Média:", round(media, 2)), 
     col = "blue", font = 1)
