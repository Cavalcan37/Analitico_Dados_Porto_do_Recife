...
MEP_km <- MEP24_full %>% 
  filter(Origem != "F. NORONHA", Destino != "F. NORONHA", Ano == 2024) %>% 
  group_by(Carga) %>% 
...
cluster_graph <- ggplot(MEP_km, aes(x = Atrac, y = SMov, colour = MEP_km[[Nome_coluna]])) +
  geom_point(size = 3) +
  geom_text(aes(label = Carga), vjust = -0.5, size = 3) +  # Adiciona os rótulos com base na coluna 'Destino'
...
cluster_graph <- ggplot(MEP_km, aes(x = Atrac, y = SMov, colour = .data[[Nome_coluna]], shape = .data[[Nome_coluna]])) +
  ...
  geom_text(aes(label = Carga), vjust = -0.5, size = 3) +  # Rótulos com base na coluna 'Berco'
  scale_colour_manual(
    name = "Cluster Group",
    values = c('red', 'orange', 'green3', 'deepskyblue', 'blue', 'darkorchid4', 'violet', 'pink1', 'tan3', 'black')[1:k]
  ) +
